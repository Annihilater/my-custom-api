...
#include "xlt_api_trade_define.h"
...
public:
     ...
     * @brief 处理订单状态通知
     *
     * 本函数用于接收简要订单状态通知；第一条回报时会触发已报；订单最后一条回报时会触发结束。
     *
     * @param session_id 会话ID。
     ...
     */
    virtual void on_raw_report(uint64_t session_id, ExchangePlatformType platform_type, void *report) {};
...
public:
    ...
    virtual int login(uint16_t client_id, const char *account_name, const char *password, const char *pub_key,
                      uint32_t biz_type,
                      uint64_t request_id, TradeResumeType resume_type = TradeResumeType::RESUME, uint64_t
                      start_sequence = 0) = 0;
    ...
    virtual int query_positions(uint64_t session_id, uint64_t request_id, char *security_code,
                                ExchangeIndex exchange_index, uint64_t algo_order_id = 0) = 0;
    ...
    *order_query_param, uint64_t algo_order_id = 0) = 0;
     ...
     * @return "0"表示成功，非"0"表示出错，此时用户可以调用get_last_error()来获取错误信息
     */
    virtual int modify_user_terminal_info(uint64_t session_id, const xlt_user_terminal_info_t* info) = 0;
